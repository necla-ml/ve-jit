...
#if NARGS == 1
      corr_result = correct_result[itrialx];
#elif NARGS == -1 // x from trialx, y from (kx==0? trialy[0]: previous correct_result);
      // y is cyclic shift of "previous outputs" (assumed correct in trialy values)
      //correct_result[i] = usefun(trialx[i], (i+NARGS<0
      //                                       ? trialy[i]
      //                                       : correct_result[i+NARGS]));
      corr_result = correct_result[itrialx]
      if(i+NARGS<0){
          r[RY] = itrialy[itrialx];
      }else{
          r[RY] = correct_result[itrialx_NARGS];
      }
#else
