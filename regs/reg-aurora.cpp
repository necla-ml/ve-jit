...
#include "reg-impl.hpp"         // flesh out reg-base.hpp declarations
...
#define BF(RB_ENUM) RB::shift(RB_ENUM)
#define MISC(INT) FLG::misc_t::mkval(INT)

// todo: use defRegFlags throughout, then switch to auto-generator the initializer-list
...
ChipRegistersAurora::ChipRegistersAurora()
    : regs{
        /* default (0) is Use=free, Cls=none(illegal), Sub=def, Vlen=def, Misc=def */
           ...
           RegisterBase{RegId( 8), BF(CLS::scalar) | MISC(reserve|preserve)},
           RegisterBase{RegId( 9), defRegFlags(RegId(9))},
           RegisterBase{RegId( 10), BF(CLS::scalar) | MISC(reserve)},
           RegisterBase{RegId( 11), BF(CLS::scalar) | MISC(reserve|preserve)},
           RegisterBase{RegId( 12), BF(CLS::scalar) | MISC(reserve)},
           RegisterBase{RegId( 13), BF(CLS::scalar) | MISC(reserve)},
           RegisterBase{RegId( 14), BF(CLS::scalar) | MISC(reserve|preserve)},
           RegisterBase{RegId( 15), BF(CLS::scalar) | MISC(reserve|preserve)},
           RegisterBase{RegId( 16), BF(CLS::scalar) | MISC(reserve|preserve)},
           RegisterBase{RegId( 17), BF(CLS::scalar) | MISC(reserve|preserve)},
