        ...
        int const vlen, FusedLoopTest& fl6t, int const verbose)
{
    ...
    string alg_descr=OSSFMT("// "<<pfx<<" no_unroll: vl"<<vl<<kernComment()
            ...
            <<"fl6_save_vl="<<fd0["fl6_save_vl"].getType());
    fp>>alg_descr;
...
std::string fl6_no_unrollY(LoopSplit const& lsii, LoopSplit const& lsjj,
        FusedLoopKernel& krn, Lpi const vlen/*=0*/,
 ...
 */
void fl6_no_unroll_split_ii( Cblock& outer, Cblock& inner, string pfx,
        int32_t const ilo, int32_t const ihi, LoopSplit const& lsjj,
 ...
 */
std::string fl6_no_unroll(int const vl00, LoopSplit const& lsii, LoopSplit const& lsjj,
        int const which/*=WHICH_KERNEL*/, int const verbose/*=1*/)
{
    ostringstream oss;

    // upper-level tree structure, above the fused-loop.
    Cunit pr("fl6_no_unroll","C",0/*verbose*/);
    ...
    if(lsii.lo > lsii.z){
        string pfx="f5a";
        fl6_no_unroll_split_ii(outer,inner,pfx, lsii.z, lsii.lo, lsjj,
                vl00, which, verbose);
    }
    if(lsii.hi > lsii.lo){
        string pfx="f5b";
        fl6_no_unroll_split_ii(outer,inner,pfx, lsii.lo, lsii.hi, lsjj,
                vl00, which, verbose);
    }
    if(lsii.end > lsii.hi){
        string pfx="f5c";
        fl6_no_unroll_split_ii(outer,inner,pfx, lsii.hi, lsii.end, lsjj,
                vl00, which, verbose);
    }
...
std::string fl6_no_unrollX(Lpi const vlen, LoopSplit const& lsii, LoopSplit const& lsjj,
        int const opt_t, int const which/*=WHICH_KERNEL*/, char const* ofname/*=nullptr*/)
{
    ostringstream oss;
    string author=OSSFMT(" fl6_no_unrollX(vlen="<<vlen<<","<<lsii<<","<<lsjj<<",...)");
    cout<<author<<endl;
    ...
    string program = fl6_no_unroll(vl00,lsii,lsjj,which,1/*verbose*/);
    oss<<program;
