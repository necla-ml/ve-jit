...
void FLKRN_check::emit(Cblock& bDef,
    ...
    if(as_function){
        ...
        if(bDefKernelFn.find("fl7_kernel_check")==nullptr){
            CBLOCK_SCOPE(fl7_kernel_check,
                    "void "
                    "\n__attribute__((noinline))"
                    "\nfl7_kernel_check(__vr const a, __vr const b, uint64_t const vl,"
                    "\n        uint64_t const cnt, uint64_t const jj,"
                    "\n        uint64_t const ilo, uint64_t const jlo)"
                    ,
                    bDefKernelFn.getRoot(),bDefKernelFn);
            if(v>=1) fl7_kernel_check
                >>"printf(\"fl7_kernel_check vl=%d cnt=%d jj=%d ilo=%d jlo=%d\\n\",(int)vl,(int)cnt,(int)jj,(int)ilo,(int)jlo);"
                    >>"fflush(stdout);";
            fl7_kernel_check
        ...
        string call=OSSFMT("fl7_kernel_check("<<vA<<","<<vB<<","<<sVL<<",cnt,jj,ilo,jlo);");
...
void FLKRN_print::emit(Cblock& bDef,
    ...
    if(!bInc.find("stdio.h")) bInc["stdio.h"]>>"#include <stdio.h>";
    auto& bDefKernelFn = bDef["..*/fns/first"];
    if(bDefKernelFn.find("fl7_kernel_print")==nullptr){
        CBLOCK_SCOPE(fl7_kernel_print,
                "void fl7_kernel_print(__vr const a, __vr const b,"
                "\n        uint64_t const vl)",bDefKernelFn.getRoot(),bDefKernelFn);
        fl7_kernel_print
            >>"int const terse=1;"
            >>"char const* linesep=\"\\n      \";"
            >>"printf(\"fl7_kernel_print(a,b,vl=%lu)\\n\",vl);"
    ...
    bKrn["prt"]<<"fl7_kernel_print("<<vA<<", "<<vB<<", "<<sVL<<");";
