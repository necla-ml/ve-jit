...
int call_addsub( Jitpage *page, unsigned long arg, unsigned long parm ){        
    ...
    printf("kernel(%016lx) --> ret1=%016lx, ret2=%016lx %d errors\n",arg,ret1,ret2,nerr);
    if(nerr) printf(" during call_addsub with arg = %lu = %ld\n",arg,(long)arg);
    if(nerr) printf(" during call_addsub with parm = %lu = %ld\n",parm,(long)parm);
    fflush(stdout);
    return nerr;
}
void test_kernel_math(char const* const cmd, unsigned long const parm, int const opt_level){
    printf("\nTest: %s\n", cmd);
    printf("        opt_level=%d\n",opt_level);
